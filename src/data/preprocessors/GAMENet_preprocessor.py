...
from core.backend.vocab import Vocab
...
class GAMENetPreprocessor(Preprocessor):
    ...
    def run(self, data: dict[str, Table]):
        ...
        medication = self._convert_ndc_to_atc(medications=medication, rxnorm_to_atc=data["rxnorm_to_atc"], ndc_to_rxnorm=data["ndc_to_rxnorm"])
    ...
    def _process_medications(self, data: Table) -> Table:
        ...
        data = data.drop(index=data[data["NDC"] == '0'].index, axis=0)
        ...
        data_new: Table = data.drop(columns=["NDC"])
        data_new = data_new.groupby(['SUBJECT_ID','HADM_ID','ICUSTAY_ID']).head(1).reset_index(drop=True)
        data = data_new.merge(right=data, on=['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'STARTDATE'])
    ...
    def _filter_diagnoses(self, diagnoses: Table, amount: int) -> Table:
        ...
        diagnoses = diagnoses.filter(lambda row: row['ICD9_CODE'] in top_codes.to_list('ICD9_CODE'))
        return diagnoses
