...
class SafeDrugPreprocessor(Preprocessor):
    ...
    def run(self, data: dict):
        self.logger.info("Starting SafeDrug preprocessing")

        medication = self._process_medications(data["medications"])
        med_lg2 = self._process_visit_lg2(medication).reset_index(drop=True)
        medication = medication.merge(
            med_lg2[["SUBJECT_ID"]], on="SUBJECT_ID", how="inner"
        ).reset_index(drop=True)
        
        medication = self._code_mapping_to_atc4(medication, data["ndc2RXCUI"], data["RXCUI2atc4"])
        with open(data["idx2drug"], "rb") as f: 
            NDCList = dill.load(f)
        medication = medication[medication.NDC.isin(list(NDCList.keys()))]
        medication = self._filter_k_most_med(medication, 300)
        self.logger.info("Completed Medication Processing")
        
        diagnoses = self._process_diagnoses(data["diagnoses"])
        self.logger.info("Completed Diagnoses Processing")
        
        procedures = self._process_procedures(data["procedures"])
        self.logger.info("Completed Procedures Processing")
        
        merged = self._merge(diagnoses, procedures, medication)
        ...
        ehr_adj, ddi_adj = self._create_adjacency_matrices(data["ddi"], data["drug2atc"], med_vocab, records)
        return {
    ...
    # ----------------------------------------------------------------------
    def _process_procedures(self, pro_pd: pd.DataFrame) -> pd.DataFrame:
    ...
    def _process_visit_lg2(self, med_pd: pd.DataFrame) -> pd.DataFrame:
        a = (
    ...
    def _filter_k_most_med(self, med_pd: pd.DataFrame, k:int=300) -> pd.DataFrame:
        med_count = (
            med_pd.groupby(by=["NDC"])
            .size()
            .reset_index()
        ...
        med_pd = med_pd[med_pd["NDC"].isin(med_count.loc[:k-1, "NDC"])]

        return med_pd.reset_index(drop=True)
    
    def _code_mapping_to_atc4(self, med_pd: pd.DataFrame, ndc2RXCUI_file, RXCUI2atc4) -> pd.DataFrame:
